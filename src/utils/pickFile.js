...
// Picks a file of the given MIME type via the system file picker and
// returns a guaranteed-readable local copy, or null if the user cancelled.
//
...
export async function pickFile({ mimeType, extension, cachePrefix }) {
  ...
  const localUri = `${FileSystem.cacheDirectory}${cachePrefix}-${Date.now()}.${extension}`;
  await FileSystem.writeAsStringAsync(localUri, base64, { encoding: FileSystem.EncodingType.Base64 });

  return { uri: localUri, name: asset.name || `document.${extension}`, size: asset.size || blob.size || 0 };
}
